...
int rev[N], W[N], inv[N], fac[N], fav[N];
void binom(int n) {
  fac[0] = 1;
...
int main() {
  ...
  poly a(n + 1);
  rep(i, 1, n) {
    a[i] = ll(i & 1 ? 1 : mod - 1) * fav[i] % mod * qpow((mod + 1) >> 1, C2(i)) % mod;
  }
  a = Inv(poly(1, 1) - a);
  rep(i, 0, n) a[i] = (ll)a[i] * qpow(2, C2(i)) % mod;
  a = Ln(a);
  rep(i, 1, n) printf("%d\n", int((ll)a[i] * fac[i] % mod));
  return 0;
}