...
const int N = 4e5 + 10, mod = 950009857;
...
void InitNTT(int n) {
   ...
   int w0 = qpow(5, (mod - 1) / len); W[0] = iW[len] = 1;
...
int Lag(poly a, int n) { //deg(a) = n
   a.erase(a.begin());
   return 1ll * Inv(Pow(a, n))[n - 1] * qpow(n, mod - 2) % mod;
}
int main() {
   int n, m;
   scanf("%d%d", &n, &m);
