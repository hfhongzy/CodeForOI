...
const int N = 524288 + 10, mod = 998244353, inv2 = (mod + 1) >> 1;
...
int main() {
  ...
  poly F = newton(n + 1);
  poly F2(n + 1);
  for(int i = 0; i * 2 <= n; i ++) F2[i * 2] = F[i];
  poly G = poly{0, inv2} * (F * F + F2); G.resize(n + 1);
  poly G2(n + 1);
  for(int i = 0; i * 2 <= n; i ++) G2[i * 2] = G[i];
  poly H = (G * G + G2) * inv2;
  for(int i = 2; i <= n; i ++) {
    printf("%d\n", H[i]);
  }
