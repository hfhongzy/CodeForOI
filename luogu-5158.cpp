...
poly p[N << 2];
void Prod(int u, int l, int r, const int *c) {
...
poly Inter(int u, int l, int r, int *y) {
   if(l == r) return poly(1, y[l]);
   int mid = (l + r) >> 1;
   return Inter(u << 1, l, mid, y) * p[u << 1 | 1] + Inter(u << 1 | 1, mid + 1, r, y) * p[u << 1];
}
poly Inter(int *x, int *y, int n) {
   Prod(1, 1, n, x); Calc(1, 1, n, x, Der(p[1]));
   for(int i = 1; i <= n; i ++) y[i] = (ll)y[i] * qpow(x[i], mod - 2) % mod;
   return Inter(1, 1, n, y);
}
int main() {
   static int x[N], y[N];
