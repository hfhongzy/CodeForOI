...
int main() {
   ...
   rep(i, 1, ex) rep(j, 1, m) {
      ...
      rep(k, 1, h[cur ^ 1].cnt) {
         ...
         if(p1 == 0 && p2 == 0) {
            if(mp[i + 1][j] && mp[i][j + 1]) h[cur].insert(mdf(mdf(st, j - 1, 1), j, 2), val);
         } else if(p2 == 0) {
            if(mp[i + 1][j]) h[cur].insert(st, val);
            if(mp[i][j + 1]) h[cur].insert(mdf(mdf(st, j - 1, 0), j, p1), val);
         } else if(p1 == 0) {
            if(mp[i + 1][j]) h[cur].insert(mdf(mdf(st, j - 1, p2), j, 0), val);
            if(mp[i][j + 1]) h[cur].insert(st, val);
         } else if(p1 == 1 && p2 == 1) {
