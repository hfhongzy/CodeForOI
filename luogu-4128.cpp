...
const int N = 64;
int n, m, p, pw[N * N], g[N][N], fac[N], fav[N], inv[N];
...
void dfs(int u, int sum) {
   if(sum == n) {
      int tot = 1; -- u;
      rep(i, 1, u) tot = (ll) tot * inv[b[i]] % p;
      int sz = 0;
      rep(i, 1, u) {
         sz ++;
         if(i == u || b[i] != b[i + 1]) {
...
int main() {
   scanf("%d%d%d", &n, &m, &p);
   pw[0] = 1;
   rep(i, 1, n * n) pw[i] = (ll) pw[i - 1] * m % p;
   fac[0] = 1;
   rep(i, 1, n) fac[i] = (ll) fac[i - 1] * i % p;
   fav[n] = qpow(fac[n], p - 2);
   per(i, n, 1) fav[i - 1] = (ll) fav[i] * i % p;
